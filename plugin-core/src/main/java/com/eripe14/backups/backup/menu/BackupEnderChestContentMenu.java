...
public class BackupEnderChestContentMenu {
    ...
    public void openInventory(Player player, Backup backup) {
        Gui gui = Gui.gui()
                .title(Legacy.title(this.config.enderChestTitle))
                .rows(this.config.enderChestRows)
                .disableAllInteractions()
                .create();

        Formatter formatter = new Formatter();
        this.config.enderChestContentStructure.forEach((slot, item) -> {
            this.setItem(gui, slot, ItemTransformer.transform(item, formatter, event -> { }));
        });

        for (ItemStack itemStack : backup.getBackupContent().getEnderChestContent().values()) {
            gui.addItem(ItemTransformer.transform(itemStack, formatter, event -> { }));
        }

        this.setItem(gui, this.config.enderChestGoBackSlot, ItemTransformer.transform(this.config.goBackItem, formatter, event -> {
